...
## 4. Interaction and lifecycle integration

- [x] 4.1 Confirm the toggle continues to be reset on puzzle completion, new puzzle, and back-to-menu (these resets already exist in `App.tsx`; verify nothing regresses).
- [x] 4.2 Confirm pause hides the board and therefore hides all highlighting (already covered by the existing pause overlay; verify by toggling pause with the highlight on).

## 5. Manual verification

- [ ] 5.1 Start the dev server (`npm run dev`) and load an in-progress puzzle.
- [ ] 5.2 Verify overview mode: toggle on, no selection — every conjugate cell shows the overview highlight.
- [ ] 5.3 Verify row partners: select a cell known to be in a row-only conjugate pair — only the row partner is emphasized in the row color; other conjugate cells are dimmed.
- [ ] 5.4 Verify column partners: same check with a column-only pair.
- [ ] 5.5 Verify box partners: same check with a box-only pair.
- [ ] 5.6 Verify multi-unit partners: find or construct a pair that is conjugate in both row and box (or column and box) and confirm the partner shows the `partner-multi` styling.
- [ ] 5.7 Verify pencil-mark precedence: set pencil marks on a cell that reduces its candidates so a previously-detected pair disappears; confirm the highlight updates immediately.
- [ ] 5.8 Verify dark mode: repeat 5.2 through 5.6 in dark theme, checking that row/column/box colors remain distinct from each other and from the bivalue (violet) and same-digit (amber) highlights.
- [ ] 5.9 Verify pause and completion: pause the game and confirm no highlights bleed through the pause overlay; complete the puzzle and confirm the toggle clears.
- [ ] 5.10 Verify mobile layout at a narrow viewport: emphasis colors remain readable, no tap-target regressions.

## 6. Validation

- [x] 6.1 Run `npm run build` and confirm the TypeScript build passes.
- [x] 6.2 Run `openspec validate enhance-conjugate-pair-highlight` and confirm the change validates cleanly.
